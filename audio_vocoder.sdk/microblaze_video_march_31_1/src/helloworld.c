...
// Init for while loop.
	...
	//Frequency Domain Waveform Variables (Before)
	int f_index = 0;
	int f_value_i = 0;
	int f_value_r = 0;
	int f_value = 0;
	...
	//Frequency Domain Waveform Variables (After)
	int v_f_index = 0;
	int v_f_value_i = 0;
	int v_f_value_r = 0;
	int v_f_value = 0;
	...
	while (1)
	{
		...
		//Set up frequency (before)
		f_index = (test[5] & 0xffffff) - 262144;
	...
	//		xil_printf("\%i: \%i: \n\r",f_index, f_value);
			...
			if(f_value > bins[f_index])
				bins[f_index] = bins[f_index] + 1;
			else if(f_value < bins[f_index])
				bins[f_index] = bins[f_index] - 20;
			if(bins[f_index] > 140)
				bins[f_index] = 140;
		...
		//Set up frequency (After)
		v_f_index = (test[8] & 0xffffff);
		v_f_value_r = test[9] & 0xffffff;
		v_f_value_i = test[10] & 0xffffff;
		v_f_value = v_f_value_r; //Sticking with real value for now.
		// f_value = sqrt(f_value_i * f_value_i + f_value_r * f_value_r);
		...
		if(OnOffSwitch != 0)
		{
			if(v_f_index < 1024)
			{
				v_f_index = v_f_index / 44;
				v_f_value = v_f_value / 120000;

	//			xil_printf("\%i: \%i: \n\r",v_f_index, v_f_value);
				if(v_f_index >= NUM_BINS)
					v_f_index = NUM_BINS - 1;
				else if(v_f_index <= 0)
					v_f_index = 0;
				...
				if(v_f_value > v_bins[v_f_index])
					v_bins[v_f_index] = v_bins[v_f_index] + 1;
				else if(v_f_value < v_bins[v_f_index])
					v_bins[v_f_index] = v_bins[v_f_index] - 20;
				if(v_bins[v_f_index] > 140)
					v_bins[v_f_index] = 140;
				else if(v_bins[v_f_index] <= 0)
					v_bins[v_f_index] = 1;
			}
			else
			{
		...
		if(OnOffSwitch == 0 || windowEnable == 0)
		{
			for(int i = 0; i < NUM_BINS; ++i)
			{
				v_bins[i] = v_bins[i] - 1;
			...
			for (int i = 0; i < NUM_BINS; ++i)
			{
				...
				{	
					if(bins[i] > d)
						...
						draw_pixel_color(start_x + 10 * i, start_y - d, 0xff);
					
					else
						draw_pixel_color(start_x + 10 * i, start_y - d, 0x00);
				}
			}
